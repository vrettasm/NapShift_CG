...
# Base (abstract) class.
class CoarseGrainChemShiftBase(object):
    ...
    def __init__(self, dir_input=None, dir_output=None, overwrite=True,
        ...
        if dir_input is None:
            ...
            self.dir_input = CoarseGrainChemShiftBase.dir_default
        else:
        ...
        if dir_output is None:
            ...
            self.dir_output = CoarseGrainChemShiftBase.dir_default
        else:
            # This will be the new location.
    ...
    def __str__(self):
        ...
        :return: a string representation of a CoarseGrainChemShiftBase object.
        """
        ...
        # Return the f-string.
        return f" CoarseGrainChemShiftBase Id({id(self)}): {new_line}" \
    ...
    def __repr__(self):
        ...
        :return: CoarseGrainChemShiftBase().
        """
        return f"CoarseGrainChemShiftBase(dir_input={self.dir_input}," \
...
# Training Class.
class CoarseGrainChemShiftTraining(CoarseGrainChemShiftBase):
    ...
    def train_models(self, validation_split=0.10, save_plots=True, verbose=False):
        ...
        :param validation_split: (float) This value is used to keep a portion of
        the training data aside while training to validate the training process.
        This is not the test set hold out.
        ...
        nn_model_cg = {atom: None for atom in TARGET_ATOMS}

        # Make sure the neural network outputs are cleared.
        nn_output_cg = {atom: None for atom in TARGET_ATOMS}
        ...
        for atom in TARGET_ATOMS:
            ...
            try:
                ...
                joblib.dump(input_scaler,
                            Path(models_path/f"CG_data_scaler_{atom}.gz"))
            ...
            # Compile the model.
            nn_model_cg[atom].compile(optimizer=optimization_alg, loss=tf.keras.losses.MSE)

            # First time instant.
            time_0 = time()

            # Fit the model.
            nn_output_cg[atom] = nn_model_cg[atom].fit(x=convert_to_tensor(x_train),
            ...
            nn_model_cg[atom].save(Path(models_path / f"CG_ann_model_{atom}.h5"),
                                   include_optimizer=False, save_format="h5")

            # Number of actual runs (epochs).
            n_epoch = len(nn_output_cg[atom].history['loss'])
            ...
            train_RMSE = np.sqrt(nn_output_cg[atom].history["loss"][-1])
            valid_RMSE = np.sqrt(nn_output_cg[atom].history["val_loss"][-1])
            ...
            df_output = DataFrame(nn_output_cg[atom].history)

            # Save to csv:
            with open(Path(results_path/f"CG_nn_output_{atom}.csv"),
                      mode='w') as f:
                df_output.to_csv(f)
            # _end_with_
            ...
            if save_plots:
                ...
                # Save the figure.
                fig.savefig(Path(plots_path/f"CG_nn_training_vs_validation_{atom}.png"),
    ...
    def __str__(self):
        ...
        :return: a string representation of a CoarseGrainChemShiftTraining object.
        """
        ...
        # Return the f-string.
        return f" CoarseGrainChemShiftTraining Id({id(self)}): {new_line}" \
...
# Predictor Class.
class CoarseGrainChemShiftPredictor(CoarseGrainChemShiftBase):
    ...
    # Object variables.
    __slots__ = ("nn_model_cg", "input_scaler_cg")
    ...
    def __init__(self, dir_model=None, dir_output=None, overwrite=True):
        ...
        self.nn_model_cg = {atom: None for atom in TARGET_ATOMS}
        ...
        for atom in TARGET_ATOMS:
            ...
            # Make the file path.
            file_path = Path(self.input_path/f"CG_ann_model_{atom}.h5")
            ...
            self.nn_model_cg[atom] = load_model(file_path, compile=False)

            # Create the Scaler filename.
            scaler_path = Path(self.input_path/f"CG_data_scaler_{atom}.gz")
    ...
    def save_to_file(self, pdb_id, aa_sequence, predictions, target_peptides, ref_peptides,
        ...
        try:
            ...
            f_name_out = Path(self.output_path/f"CG_prediction_{pdb_id}_"
                    ...
                    for atom in TARGET_ATOMS:
                        ...
                        if search_peptide in target_peptides[atom]:
                            ...
                            if random_coil is not None:
                                ...
                                rc_value = CoarseGrainChemShiftPredictor.df_avg.loc[atom, res_name]
                            # _end_if_
    ...
    def predict(self, f_path, n_peptides=3, random_coil_path=None, verbose=False, talos_fmt=True):
        """
        It accepts a PDB file as input, constructs the input to the trained NN and puts the results
        (predicted chemical shifts) in a new text file.
        ...
        The default setting is 'False' to avoid cluttering the screen with information.

        :param talos_fmt: (bool) If "True" (default) it will use the TALOS format to save the results.
        If it is set to 'False' the output format will be tabular.
        ...
        model_vectors = CoarseGrainChemShiftPredictor.r_vec_in(f_path,
        ...
        for model_id, model_value in enumerate(model_vectors.values(), start=0):
            ...
            for chain_id, chain_seq in amino_acid_seq.items():
                ...
                if random_coil_shifts is not None:
                    ...
                    df_random_coil = CoarseGrainChemShiftPredictor.random_coil(chain_seq)
                # _end_if_
                ...
                for atom in TARGET_ATOMS:
                    ...
                    y_predict[atom] = self.nn_model_cg[atom].predict(x=convert_to_tensor(df_data),
    ...
    def __str__(self):
        ...
        :return: a string representation of a CoarseGrainChemShiftPredictor object.
        """
        ...
        # Return the f-string.
        return f" CoarseGrainChemShiftPredictor Id({id(self)}): {new_line}" \
